exports.run = async(bot, message, args) => {
    ...
    var output = "";
    var result = "";
    const tokens = await bot.db.getUser(message.author.id).then(res => {
        return res[0].tokens;
    }).catch(err => {
        bot.LogIt.error(err);
    });

    if (isNaN(dices) || (dices <= 0 && dices > 7)) return message.reply('Tienes que introducir un número de dados válido [0-6]');

    if (isNaN(bet)) return message.reply('La cantidad de la apuesta tiene que ser un número');

    if (isNaN(guessNum) || (guessNum <= 0 && guessNum > 11)) return message.reply('El número de dados tiene que ser un número [1-10]')

    if (tokens < bet) return message.reply(`La apuesta es mayor que el número de tokens, tienes ${tokens} tokens`);
